...
function useReportData() {
    ...
    const refresh = useCallback(async () => {
        try {
            const [shipments, containers, expenses] = await Promise.all([
...
function buildAnalyticsReports(data) {
    const countryImportsData = Object.values(data.shipments.reduce((acc, shipment) => {
        ...
        acc[country] || (acc[country] = { country, shipments: 0 });
...
function AnalyticsReports({ data, loading }) {
    ...
    const hasTrends = monthlyTrendsData.some((row) => row.shipments > 0 || row.costs > 0);
