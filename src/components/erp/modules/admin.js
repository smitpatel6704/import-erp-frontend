...
const mockUsers = [
    ...
    { id: '3', email: 'mike@importerp.com', name: 'Mike Johnson', role: 'user', department: 'Operations', isActive: true, lastLoginAt: new Date(Date.now() - 7200000).toISOString(), createdAt: '2024-02-15T00:00:00Z' },
...
const formatAuditEntity = (entity) => String(entity || '-').replace(/_/g, ' ').replace(/\b\w/g, (letter) => letter.toUpperCase());
...
function ConfigurableList({ category, title, icon: Icon }) {
    ...
    return (_jsxs(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, children: [_jsxs(Card, { className: "shadow-sm h-full", children: [_jsxs(CardHeader, { className: "pb-3", children: [_jsxs(CardTitle, { className: "text-base font-semibold flex items-center gap-2", children: [_jsx(Icon, { className: "h-4 w-4 text-teal" }), title] }), _jsx(CardDescription, { className: "text-xs", children: "Manage dropdown options" })] }), _jsxs(CardContent, { children: [_jsxs("div", { className: "flex gap-2 mb-4", children: [_jsx(Input, { value: newValue, onChange: (e) => setNewValue(e.target.value), placeholder: `Add new...`, className: "h-8 text-xs", onKeyDown: (e) => e.key === 'Enter' && handleAdd() }), _jsxs(Button, { onClick: handleAdd, size: "sm", className: "h-8 text-xs shrink-0", children: [_jsx(Plus, { className: "h-3.5 w-3.5 mr-1" }), " Add"] })] }), _jsx("div", { className: "space-y-2 max-h-[250px] overflow-y-auto custom-scrollbar pr-2", children: options.length === 0 ? (_jsx("p", { className: "text-xs text-muted-foreground text-center py-4", children: "No options found." })) : (options.map((opt) => {
        const isSystemShippingLine = category === 'shipping_line' && ['EVERGREEN', 'Hapag-Lloyd', 'Maersk', 'MSC'].map(s => s.toLowerCase()).includes(opt.label?.toLowerCase());
        return (_jsxs("div", { className: "flex items-center justify-between p-2 border border-border/50 rounded-md text-xs bg-muted/20", children: [_jsx("span", { className: "font-medium", children: opt.label }), !isSystemShippingLine && _jsx(Button, { variant: "ghost", size: "sm", className: "h-6 w-6 p-0 text-destructive hover:text-destructive hover:bg-destructive/10", onClick: () => setDeleteId(opt.id), children: _jsx(Trash2, { className: "h-3.5 w-3.5" }) })] }, opt.id));
    })) })] })] }),
    _jsx(Dialog, { open: deleteId !== null, onOpenChange: (open) => !open && setDeleteId(null), children: _jsxs(DialogContent, { className: "max-w-xs", children: [_jsx(DialogHeader, { children: _jsx(DialogTitle, { className: "text-sm", children: "Confirm Deletion" }) }), _jsx("div", { className: "py-2 text-xs text-muted-foreground", children: "Are you sure you want to delete this? This action cannot be undone." }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", size: "sm", className: "h-8 text-xs", onClick: () => setDeleteId(null), children: "Cancel" }), _jsx(Button, { variant: "destructive", size: "sm", className: "h-8 text-xs", onClick: () => { handleDelete(deleteId); setDeleteId(null); }, children: "Delete" })] })] }) })
    ] }));
}
