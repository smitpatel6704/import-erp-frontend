...
function ShipmentChecklistModal({ shipment, shipments, open, onOpenChange, onNavigate, onRefresh }) {
        ...
        <Dialog open={exportOpen} onOpenChange={(o) => !o && !merging && setExportOpen(false)}>
            <DialogContent className="max-w-md">
                ...
                <DialogFooter>
                    ...
                    <Button size="sm" className="bg-primary hover:bg-primary/90" disabled={merging || selectedMergeIds.length < 2} onClick={handleMergeDocuments}>
    ...
    if (!shipment)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx(Dialog, { open: open, onOpenChange: onOpenChange, children: _jsxs(DialogContent, { className: "max-w-2xl h-[82vh] max-h-[720px] flex flex-col p-0 overflow-hidden", children: [_jsx(DialogHeader, { className: "shrink-0 px-4 py-3 border-b bg-muted/20", children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Button, { variant: "ghost", size: "sm", className: "h-8 w-8 p-0", disabled: !hasPrev, onClick: () => prevShipment && onNavigate(prevShipment), title: prevShipment ? `Previous: ${prevShipment.shipmentNumber}` : 'No previous shipment', children: _jsx(ChevronLeft, { className: "h-4 w-4" }) }), _jsx(Button, { variant: "ghost", size: "sm", className: "h-8 w-8 p-0", disabled: !hasNext, onClick: () => nextShipment && onNavigate(nextShipment), title: nextShipment ? `Next: ${nextShipment.shipmentNumber}` : 'No next shipment', children: _jsx(ChevronRight, { className: "h-4 w-4" }) })] }), _jsxs("div", { children: [_jsxs(DialogTitle, { className: "text-xl flex items-center gap-2", children: [_jsx(Ship, { className: "h-5 w-5 text-teal-600" }), shipment.shipmentNumber] }), _jsxs(DialogDescription, { className: "text-xs mt-1", children: ["Document Checklist Tracking \u2022 ", (_a = shipment.company) === null || _a === void 0 ? void 0 : _a.name] })] })] }), _jsxs("div", { className: "text-right", children: [_jsxs(Badge, { variant: "outline", className: "text-[10px] font-mono mb-1", children: [checklist.filter(c => c.status === 'verified').length, " / ", checklist.filter(c => c.isRequired).length, " Verified"] }), _jsx("p", { className: "text-[10px] text-muted-foreground uppercase tracking-tighter", children: "Required Docs" })] })] }) }), _jsx(ScrollArea, { className: "flex-1 min-h-0 px-4 py-3", children: loading ? (_jsx(ShippingLoader, { className: "py-20", compact: true, label: "Loading document checklist" })) : (_jsx("div", { className: "space-y-4", children: ['booking', 'arrival', 'clearance', 'delivery'].map((stage) => {
